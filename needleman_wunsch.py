"""
Needleman-Wunsch (NW) algorithm for comparison of two sequences (global alignment).
...
class NeedlemanWunsch:
    ...
    def __init__(self, submatrix_file: str = "submatrix.csv", GP: int = -2) -> None:
    ...
    def compute_auxillary_matrices(self, seqA: str, seqB: str) -> None:
        """
        ...
        for i in range(1, rowdim):
            for j in range(1, coldim):
                ...
                # choose the maximum score and update the score matrix
    ...
    def _create_initial_direction_matrix(
        ...
        """
        Creates a direction matrix for the NW algorithm.
    ...
    def _create_initial_score_matrix(self, rowdim: int, coldim: int) -> np.ndarray:
        """
        Creates a score matrix for the NW algorithm.
        ...
        score_matrix[0, :] = np.arange(0, coldim) * self.GP
        score_matrix[:, 0] = np.arange(0, rowdim) * self.GP
        return score_matrix
...
if __name__ == "__main__":
    ...
    nw.compare("TATA", "ATAT", n=1, output_file="output.txt")
